...
client.on("ready", async () => {
  setInterval(() => {
    ...
    client.user.setPresence({ activity: { name: `${conf.durum[index]}` }, status: "dnd" });
  }, 15000);
...
client.on("emojiUpdate", async (oldEmoji, newEmoji) => {
  ...
  if (emojiKoruma[entry.executor.id] >= 5) {
...
client.on("message", async message => {
  ...
  if (message.mentions.everyone || message.mentions.roles.some(r => r.members.size > 500) || message.mentions.users.size > 30) {
    ...
    if (etiketKoruma[message.author.id] >= 3) {
      Punish(message.author.id, "jail");
      Webhook.send(conf.ETİKET, new MessageEmbed().setDescription(`────────────────────────\n**${message.author}** (\`${message.author.id}\`) **Everyone atma limtini geçtiği için jaile atıldı.**\n\n────────────────────────`)).catch(console.error);;
    };
...
client.on("message", async message => {
  ...
  let embed = new MessageEmbed().setColor("2F3136").setAuthor(message.member.displayName, message.author.avatarURL({
    dynamic: true,
  })).setFooter(`${client.users.cache.has(conf.botOwner) ? client.users.cache.get(conf.botOwner).tag : "Eray"} was here!`).setTimestamp();
  ...
  if (command === "güvenli2") {
    ...
    if (uye) hedef = uye;
    let guvenliler = ayarlar.güvenli2 || [];
    if (!hedef) return message.channel.send(new MessageEmbed().setFooter(`Eray Guard System`).setFooter("Yapabildikleri : Bir çok şeyi yapabilirsiniz owner guard tarafından denetlenmektesiniz, abartılı işlemlerde cezalandırma uygulanır.").setDescription(`\`\`\`Güvenli listeye eklemek/kaldırmak için bir hedef (rol/üye) belirtmelisin!\`\`\``).addField("Güvenli Liste", guvenliler.length > 0 ? guvenliler.map(g => (message.guild.roles.cache.has(g.slice(1)) || message.guild.members.cache.has(g.slice(1))) ? (message.guild.roles.cache.get(g.slice(1)) || message.guild.members.cache.get(g.slice(1))) : g).join('\n') : "Bulunamadı!"));
    if (guvenliler.some(g => g.includes(hedef.id))) {
      guvenliler = guvenliler.filter(g => !g.includes(hedef.id));
  ...
  if (command === "rol") {
    ...
    if (!hedef) return message.channel.send(new MessageEmbed().setColor("RANDOM").setFooter(`Eray Guard System`)
      .setDescription(`\`\`\`Güvenli rolleri belirtmek için g!rol rolid/roletiket yapmanız yeterlidir.\`\`\``).addField("Güvenli Rol Listesi", guvenliler.length > 0 ? guvenliler.map(g => (message.guild.roles.cache.has(g) || message.guild.members.cache.has(g)) ? (message.guild.roles.cache.get(g) || message.guild.members.cache.get(g)) : g).join('\n') : "Bulunamadı!"));
    if (guvenliler.some(g => g.includes(hedef.id))) {
      guvenliler = guvenliler.filter(g => !g.includes(hedef.id));
